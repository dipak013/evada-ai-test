...
export function AgentManagementTab({ selectedClient, selectedAgent }: Readonly<AgentManagementTabProps>) {
  ...
  const selectedManagedAgent = filteredManagedAgents[0] ?? null;
  ...
  }, [agentStatuses, filteredManagedAgents]);
  ...
  useEffect(() => {
    const loadLiveAgentData = async () => {
      ...
      try {
        ...
        setAgentStatuses(
          Object.fromEntries(statusesResponse.data.agents.map((row) => [row.agent_id, row]))
        );
      } catch (error) {
        console.error("Failed to load client agent data", error);
  ...
  const handleDownloadAgent = async () => {
    if (!selectedOs) {
      setDownloadError("Please select an operating system before downloading.");
      return;
    }
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/agent-download/${selectedOs.toLowerCase()}/`, {
        method: "GET",
        credentials: "include",
      });
      ...
      const plainNameMatch = /(?:^|;)\s*filename="?([^";]+)"?/i.exec(contentDisposition);
      const encodedNameMatch = /(?:^|;)\s*filename\*=UTF-8''([^;]+)/i.exec(contentDisposition);
      const plainName = plainNameMatch?.[1]?.trim();
      const encodedName = encodedNameMatch?.[1]?.trim();
      const selectedName = plainName || encodedName || "";
      const fileName = selectedName ? decodeURIComponent(selectedName) : "";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-6">
            ...
            >
              {isDownloading ? "Preparing download..." : `Download sentinel agent - ${selectedOs}`}
            </button>
            {downloadError && (
            ...
            <div className="rounded-lg border border-gray-200 bg-slate-50 p-4 mb-4">
              <h4 className="text-sm font-semibold text-gray-900">Current Agent License</h4>
              <p className="text-xs text-gray-600 mt-1 mb-3">
                {selectedManagedAgent
                  ? `${selectedManagedAgent.agentName} · ${selectedManagedAgent.clientName ?? "Unassigned client"}`
                  : "No active agent selected for this client."}
              </p>

              {selectedManagedAgent ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700">
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">License Key</div>
                    <div className="mt-1 font-mono text-xs break-all">{selectedManagedAgent.licenseKey ?? "Not assigned"}</div>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">Agent UUID</div>
                    <div className="mt-1 font-mono text-xs break-all">{selectedManagedAgent.agentUuid ?? "Pending binding"}</div>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">Tier</div>
                    <div className="mt-1">{selectedManagedAgent.licenseTier ?? "—"}</div>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">Device Limit</div>
                    <div className="mt-1">{selectedManagedAgent.deviceLimit ?? "—"}</div>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">License Status</div>
                    <div className="mt-1">{selectedManagedAgent.licenseActive ? "Active" : "Inactive"}</div>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <div className="text-xs uppercase tracking-wide text-gray-500">Expires At</div>
                    <div className="mt-1">{selectedManagedAgent.licenseExpiresAt ? new Date(selectedManagedAgent.licenseExpiresAt).toLocaleString() : "Not set"}</div>
                  </div>
                </div>
              ) : (
                <div className="text-xs text-gray-500">No license mapping is available yet.</div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <ol className="text-xs text-gray-600 space-y-1">
                  {selectedOs === "Windows" && (
                    <>
                      <li>1. Run the installer as Administrator</li>
                      <li>2. Follow the installation wizard</li>
                      <li>3. Enter your agent key when prompted</li>
                      <li>4. Agent will auto-start after installation</li>
                    </>
                  )}
                  {selectedOs === "Linux" && (
                    <>
                      <li>1. Extract the downloaded archive</li>
                      <li>2. Run: sudo ./install.sh</li>
                      <li>3. Configure agent with your API key</li>
                      <li>4. Start service: sudo systemctl start sentinel-agent</li>
                    </>
                  )}
                  {selectedOs === "macOS" && (
                    <>
                      <li>1. Open the downloaded .pkg file</li>
                      <li>2. Follow the installation prompts</li>
                      <li>3. Grant necessary permissions in System Preferences</li>
                      <li>4. Configure agent via terminal or GUI</li>
                    </>
                  )}
                </ol>
              </div>
