...
const contactChannels = [
  {
    ...
    Icon: marketingIconMap["contact-email"],
  },
  {
    title: "Phone",
    detail: "02039166414 / 07723115384",
    href: "tel:02039166414",
    Icon: marketingIconMap["contact-phone"],
  },
  {
    title: "Address",
    detail: "124 City Road, London, EC1V 2NX",
    Icon: marketingIconMap["contact-location"],
  },
];

const demoHighlights = [
  { label: "Platform questions", Icon: marketingIconMap["show-workflow"] },
  { label: "Security enquiries", Icon: marketingIconMap["contact-message"] },
  { label: "Governance support", Icon: marketingIconMap["enterprise-governance"] },
  { label: "Team response", Icon: marketingIconMap["human-expertise"] },
];

const expectationItems: ExpectationItem[] = [
  {
    text: "We review your enquiry and a member of the EVADA team assesses your requirements.",
    Icon: marketingIconMap["enterprise-governance"],
  },
  {
    text: "We schedule a short call to discuss your environment, current tooling and validation goals.",
    Icon: marketingIconMap["schedule-call"],
  },
  {
    text: "We show the right workflow for controlled validation, evidence capture and remediation.",
    Icon: marketingIconMap["show-workflow"],
  },
  {
    text: "We agree a practical rollout path for your team.",
        ...
        <div className="relative mx-auto grid w-full max-w-[1360px] items-center gap-8 lg:grid-cols-[0.44fr_0.56fr] lg:gap-12">
          <div className="w-full max-w-[620px]">
            ...
            <h1 className="mt-5 max-w-full break-words text-[34px] font-bold leading-[1.07] tracking-[-0.035em] text-[#071633] sm:text-[46px] lg:text-[clamp(3rem,4.55vw,3.75rem)]">
              Speak to an EVADA <span className="bg-[linear-gradient(90deg,#2563EB,#06B6D4,#7C3AED)] bg-clip-text text-transparent">security specialist.</span>
            </h1>
            <p className="mt-5 w-full max-w-full text-[15px] leading-[1.7] text-slate-600 sm:max-w-[540px] sm:text-[16px]">
              Whether you are exploring continuous validation, AI-supported pentest workflows, agent deployment or enterprise governance, our team can help you plan the right approach.
            </p>
          ...
          <div className="relative w-full">
            ...
            <div className="relative overflow-hidden rounded-[28px] border border-blue-100 bg-white p-5 shadow-[0_28px_80px_rgba(37,99,235,0.14)] sm:p-7">
              <div className="flex flex-col gap-4 border-b border-blue-100 pb-5 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p className="text-[12px] font-black uppercase tracking-[0.18em] text-[#2563EB]">Send an enquiry</p>
                  <h2 className="mt-2 text-[clamp(1.55rem,4vw,2rem)] font-bold leading-[1.1] tracking-[-0.025em] text-[#071633]">Tell us what you need</h2>
                  <p className="mt-2 max-w-[580px] text-[14px] font-normal leading-relaxed text-slate-600">
                    Provide your details and enquiry context. We will prepare a focused response from the right EVADA specialist.
                  </p>
                </div>
        ...
        <section className="bg-white px-4 py-12 sm:px-8 lg:px-10">
          <div className="mx-auto grid max-w-[1360px] gap-6 rounded-[26px] border border-blue-100 bg-[linear-gradient(135deg,#EFF6FF_0%,#FFFFFF_48%,#F5F3FF_100%)] p-6 shadow-[0_22px_60px_rgba(37,99,235,0.10)] md:grid-cols-[0.36fr_0.64fr] md:p-8">
            <div>
              ...
              <h2 className="mt-3 max-w-[420px] text-[clamp(1.55rem,4vw,2.35rem)] font-bold leading-[1.1] tracking-[-0.025em] text-[#071633]">What happens next</h2>
            </div>
            <div className="grid gap-3">
              {expectationItems.map(({ text, Icon }) => (
                <div key={text} className="flex min-w-0 gap-3 rounded-2xl bg-white/84 p-4 text-[15px] font-semibold leading-relaxed text-slate-700 ring-1 ring-blue-100">
                  <Icon aria-hidden="true" className="mt-0.5 h-5 w-5 shrink-0 text-[#2563EB]" strokeWidth={2.2} />
                  <span>{text}</span>
                </div>
              ))}
            </div>
